...
app.controller('hallController' ,function($scope,$controller,cinemaService   ,hallService){
	...
	$scope.creatSeat=function () {
		var sList = new Array();
		var id = 1;
		for (var i = 1; i <= $scope.room.x;i++){
			for (var j = 1; j <= $scope.room.y;j++){
				var obj = new Object();
				obj.x=i;obj.y=j;obj.id=id;obj.status=1;
				sList.push(obj);
				id++;
			}
		}
		$scope.seatObj.column=$scope.room.y;
		$scope.seatObj.row=$scope.room.x;
		$scope.seats = sList;
		$scope.seatObj.seats=sList;
		$scope.entity.seat=$scope.seatObj;
	}
});	